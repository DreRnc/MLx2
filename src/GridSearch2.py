...
class GridSearch():
    ...
    Attributes
    ----------
    self.model (MLP, MultioutputSVR): the model to be optimized, after the fit method is called it contains the best model found
    ...
    self.best_model (MLP, SVR): the best model found, trained on the whole dataset provided
    ...
    def print_grid_results(self, n_results = 1):
        '''
        Prints the results of the grid search
        ...
        if n_results > 1:
            for i in range(0, min(n_results, len(results))):
                print('\n')
                print(f'Best {i+1} parameters: {results[i][1]}')
                print(f'Best {i+1} score: {results[i][0]}')
